...
export function createHostEntity(data: HostAsset) {
  ...
  return createIntegrationEntity({
    entityData: {
      ...
      assign: {
        ...
        modified: parseTimePropertyValue(data.modified),
        type: data.type,
        qwebHostId: data.qwebHostId,
        lastVulnScan: parseTimePropertyValue(data.lastVulnScan),
        fqdn: data.fqdn,
        dnsHostName: data.dnsHostName,
        address: data.address,
        trackingMethod: data.trackingMethod,
        isDockerHost: data.isDockerHost,
      },
    },
  });
}
